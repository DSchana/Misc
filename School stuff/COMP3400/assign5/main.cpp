...
using invhist_t = std::map<std::size_t, std::list<record> >;
...
int main() {
    ...
    for (auto const& [key, value] : h) {
        std::list<record> rs = { key };  // list to set or merge at 'value'
        auto i_ret = ih.insert(std::pair<size_t, std::list<record> >(value, rs));

        if (!i_ret.second) {
            i_ret.first->second.push_back(key);
        }
    }

    for (auto const& [key, value] : ih) {
        int out_count = 0;

        std::cout << key << ": { ";
        for (auto v : value)
            std::cout << v.fname << " " << v.lname << (++out_count == value.size() ? " " : ", ");
        std::cout << "}" << std::endl;
    }
